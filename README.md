...
[![Build Status](https://img.shields.io/github/checks-status/leviable/go-template-test/main?color=black&style=for-the-badge&logo=github)][github-actions]
[![Code Coverage](https://img.shields.io/codecov/c/github/leviable/go-template-test?color=blue&logo=codecov&style=for-the-badge)][github-actions-tests]
...
[![License](https://img.shields.io/github/license/leviable/go-template-test?color=red&style=for-the-badge)][project-license]
...
```sh
git clone https://github.com/leviable/go-template-test
cd go-template-test
```
</details>
...
This section will guide you to setup a fully-functional local copy of `go-template-test`
...
from [`go-template-test's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `go-template-test`, run the
command
...
The pre-commit hooks used by `go-template-test` are located within the
...
Tests in `go-template-test` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *go-template-test*                           	|       NA      	|
|  `docker-gen`  	| Create a new docker image for *go-template-test* 	|     docker    	|
...
```text
go-template-test_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
go-template-test_v1.2.1_windows_x86_64.exe
```
...
To run `go-template-test` in a docker container, read the instructions in
[docker section](./docker).

### Running `go-template-test`

To run go-template-test, use the command
